...
_EMPTY_ORDER_ID_KEY = "__EMPTY_ORDER_ID__"
...
def get_already_processed_ids():
    ...
    try:
        ...
        ids |= {r.order_id if r.order_id else _EMPTY_ORDER_ID_KEY for r in rows}
...
def main():
    if STREAM_LATE_OFFSET_SECONDS:
        ...
        logger.info("STREAM_LATE_OFFSET_SECONDS=0 -> publishing events with current timestamp (not late).")

    already_processed = get_already_processed_ids()
    rows = read_stream_orders()
    sent, skipped, failed = 0, 0, 0
    for row in rows:
        row_key = row["order_id"] if row["order_id"] else _EMPTY_ORDER_ID_KEY
